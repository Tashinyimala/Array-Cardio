const inventors = [
  ...
  const people = ['Beck, Glenn', 'Becker, Carl', 'Beckett, Samuel',
   'Beddoes, Mick', 'Beecher, Henry', 'Beethoven, Ludwig', 'Begin, Menachem',
    ...
    // const fifteen100 = inventors.filter(function(inventor) {
    //     if(inventor.year >= 1500 && inventor.year <=1599) {
    //         return true;
    //     }
    // });

    // console.table(fifteen100);
    ...
    const friends = [
      ...
      const isAdult = friends.some(friend => {
        const currentYear = new Date().getFullYear();
        return currentYear - friend.year >= 19;
      });
      ...
      const everyAdult = friends.every(friend => (new Date().getFullYear()) - friend.year >= 19);
